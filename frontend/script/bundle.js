var app = (() => {
  ...
  function handleCellEdit(inputElement, rowIndex, header) {
    ...
    const oldValue = (APP_STATE2.previewRows[rowIndex][header] || "").trim();
    if (newValue !== oldValue) {
      APP_STATE2.previewRows[rowIndex][header] = newValue;
      APP_STATE2.hasUnvalidatedChanges = true;
      updateDryRunButton();
      document.getElementById("start-real-import-btn").disabled = true;
    }
    const mappedPath = APP_STATE2.mapping[header] || "";
  ...
  // frontend/script/loaders/dashboard.js
  async function loadDashboard() {
    try {
      ...
      let customers = [];
      let jobs = [];
      let salesData2 = { total: 0, monthly: [] };
      ...
      if (reportsRes.status === "fulfilled" && reportsRes.value.ok) {
        salesData2 = await reportsRes.value.json();
      }
      const html = `
      <div class="fade-in">
          ...
          <div class="bg-white p-6 rounded-xl shadow">
            <div class="flex items-center">
              ...
              <div>
                ...
                <p class="text-3xl font-bold">$${salesData2.total?.toLocaleString() || "0"}</p>
              </div>
            </div>
          </div>
  ...
  // frontend/script/loaders/customers.js
  async function loadCustomersSection(subSection) {
    try {
      ...
      if (subSection === "list" || !subSection) {
        ...
        if (res.ok) {
          APP_STATE2.customers = await res.json();
        }
        html = `
        <div class="fade-in">
          <div class="flex justify-between items-center mb-8">
              ...
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    ${APP_STATE2.customers.map((customer) => `
      ...
      if (subSection === "import") {
        APP_STATE2.currentJobId = null;
        APP_STATE2.hasUnvalidatedChanges = false;
        APP_STATE2.csvHeaders = [];
        APP_STATE2.previewRows = [];
        APP_STATE2.mapping = {};
        APP_STATE2.currentValidation = {};
        APP_STATE2.overrideDuplicates = false;
        APP_STATE2.qboDuplicateOnlyRows = 0;
        APP_STATE2.lastDryRunSummary = null;
        APP_STATE2.lastDryRunRows = null;
      }
    } catch (error) {
  ...
  // frontend/script/loaders/products.js
  async function loadProductsSection(subSection) {
    try {
      ...
      if (subSection === "list" || !subSection) {
        ...
        if (res.ok) {
          APP_STATE2.products = await res.json();
        }
        html = `
        <div class="fade-in">
          <div class="flex justify-between items-center mb-8">
            ...
            ${APP_STATE2.products.length > 0 ? `
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    ${APP_STATE2.products.map((product) => `
                      <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
  ...
  // frontend/script/loaders/reports.js
  async function loadReportsSection(subSection) {
    try {
      ...
      `;
        ...
        let salesData2 = { summary: {}, monthly: [] };
        if (res.ok) {
          salesData2 = await res.json();
        }
        html = `
        <div class="fade-in">
          <div class="flex justify-between items-center mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white p-6 rounded-xl shadow">
              ...
              <p class="text-3xl font-bold text-green-600">$${salesData2.summary?.totalSales?.toLocaleString() || "0"}</p>
            </div>
            <div class="bg-white p-6 rounded-xl shadow">
              <p class="text-gray-500 text-sm">Total Orders</p>
              <p class="text-3xl font-bold text-blue-600">${salesData2.summary?.totalOrders || "0"}</p>
            </div>
            <div class="bg-white p-6 rounded-xl shadow">
              <p class="text-gray-500 text-sm">Average Order</p>
              <p class="text-3xl font-bold text-purple-600">$${salesData2.summary?.averageOrder?.toFixed(2) || "0.00"}</p>
            </div>
            <div class="bg-white p-6 rounded-xl shadow">
              <p class="text-gray-500 text-sm">Top Customer</p>
              <p class="text-3xl font-bold text-orange-600">${salesData2.summary?.topCustomer || "\u2014"}</p>
            </div>
          </div>
  ...
  // frontend/script/loaders/jobs.js
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ${jobs.map((job) => `
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="px-3 py-1 text-xs rounded-full ${job.status === "completed" ? "bg-green-100 text-green-800" : job.status === "failed" ? "bg-red-100 text-red-800" : job.status === "processing" ? "bg-blue-100 text-blue-800" : "bg-yellow-100 text-yellow-800"}">
                        ${job.status}
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="text-gray-900">Total: ${job.total_rows || "?"}</div>
                      ${job.failed_rows !== void 0 ? `<div class="text-sm text-red-600">Failed: ${job.failed_rows}</div>` : ""}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
  ...
  // frontend/script/navigation.js
  async function loadSection2(section, subSection = null) {
    APP_STATE2.currentSection = section;
    APP_STATE2.currentSubSection = subSection;
  ...
  function updateSidebar(section, subSection) {
    ...
    sidebarNav.querySelectorAll("a").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        loadSection2(section, subsection);
      });
    });
  }

  // frontend/script/import.js
  async function uploadForPreview() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
      APP_STATE2.currentJobId = data.job_id;
      APP_STATE2.csvHeaders = data.headers || [];
      APP_STATE2.previewRows = data.preview_rows || [];
  ...
  function renderMappingFields() {
    ...
    container.innerHTML = APP_STATE2.csvHeaders.map((header) => `
  ...
  function proceedToPreview() {
    ...
    document.querySelectorAll(".mapping-row").forEach((row) => {
      ...
      if (header && select.value) {
        APP_STATE2.mapping[header] = select.value;
      }
    });
  ...
  function renderPreviewTableClean() {
    ...
    <tr>
      ...
      ${APP_STATE2.csvHeaders.map((h) => `<th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">${h}</th>`).join("")}
    </tr>`;
    tbody.innerHTML = APP_STATE2.previewRows.map((row, idx) => `
    <tr class="hover:bg-gray-50 transition-colors">
      ...
      ${APP_STATE2.csvHeaders.map((header) => {
      const mappedPath = APP_STATE2.mapping[header] || "";
  ...
  function updateDryRunButton2() {
    const btn = document.getElementById("dry-run-btn");
    if (APP_STATE2.hasUnvalidatedChanges) {
      btn.textContent = "Validate Changes \u26A1";
      btn.classList.add("animate-pulse", "ring-4", "ring-indigo-300", "bg-indigo-700");
    } else {
      btn.textContent = APP_STATE2.currentValidation ? "Re-run Dry Run" : "Run Dry Run Simulation";
      btn.classList.remove("animate-pulse", "ring-4", "ring-indigo-300", "bg-indigo-700");
    }
  }
  function updateSummaryDisplay() {
    if (!APP_STATE2.lastDryRunSummary) return;
    const s = APP_STATE2.lastDryRunSummary;
    const override = APP_STATE2.overrideDuplicates;
    const effectiveSucceed = override ? s.will_succeed + APP_STATE2.qboDuplicateOnlyRows : s.will_succeed;
    ...
    if (effectiveZeroFail) {
      document.getElementById("summary-text").textContent = "\u{1F389} Ready to import!";
  ...
  function renderPreviewTableWithValidation(validationRows) {
    APP_STATE2.lastDryRunRows = validationRows;
    APP_STATE2.currentValidation = {};
    validationRows.forEach((r) => {
      APP_STATE2.currentValidation[r.row_number] = {
        status: r.status,
        issues: r.issues || []
      };
    });
    ...
    validationRows.forEach((r) => {
      if (r.status === "error") {
        ...
        if (!hasNonQboDup && hasQboDup) APP_STATE2.qboDuplicateOnlyRows++;
      }
    });
    APP_STATE2.hasUnvalidatedChanges = false;
    updateDryRunButton2();
    ...
    let html = "";
    APP_STATE2.previewRows.forEach((row, idx) => {
      const rowNum = idx + 2;
      const val = APP_STATE2.currentValidation[rowNum] || { status: "valid", issues: [] };
      let filteredIssues = val.issues;
      if (APP_STATE2.overrideDuplicates) {
      ...
      const allIssues = filteredIssues.map((iss) => {
        ...
        let niceField = field.replace("PrimaryEmailAddr.Address", "Email").replace("PrimaryPhone.FreeFormNumber", "Phone").replace("Mobile.FreeFormNumber", "Mobile").replace("WebAddr.URI", "Website").replace("BillAddr.", "Billing ").replace("ShipAddr.", "Shipping ");
      ...
      const rowClass = effectiveStatus === "valid" ? "bg-white hover:bg-green-50 border-l-4 border-green-500" : "bg-white hover:bg-red-50 border-l-4 border-red-500";
      ...
      ${APP_STATE2.csvHeaders.map((header) => {
        const mappedPath = APP_STATE2.mapping[header] || "";
        const fieldIssues = filteredIssues.filter(
          (iss) => iss.field && (iss.field === mappedPath || iss.field.endsWith("." + mappedPath.split(".").pop()) || mappedPath.includes(iss.field))
        );
  ...
  async function startDryRun() {
    if (!APP_STATE2.mapping || Object.keys(APP_STATE2.mapping).length === 0) {
      alert("Please complete mapping first");
      return;
    }
    const payload = {
      mapping: APP_STATE2.mapping,
      rows: APP_STATE2.previewRows
    };
    try {
      const res = await fetchWithAuth(`/api/import/customer/${APP_STATE2.currentJobId}/dry-run`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      APP_STATE2.lastDryRunSummary = s;
      APP_STATE2.lastDryRunRows = data.rows;
      ...
      APP_STATE2.overrideDuplicates = document.getElementById("override-duplicates").checked;
      renderPreviewTableWithValidation(data.rows);
      document.getElementById("override-duplicates").onchange = () => {
        APP_STATE2.overrideDuplicates = document.getElementById("override-duplicates").checked;
        if (APP_STATE2.lastDryRunRows) {
          renderPreviewTableWithValidation(APP_STATE2.lastDryRunRows);
        }
      };
    } catch (err) {
  ...
  async function startRealImport() {
    ...
    try {
      const res = await fetchWithAuth(`/api/import/customer/${APP_STATE2.currentJobId}/start`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".sidebar-link")) {
      ...
      const subsection = link.dataset.subsection;
      loadSection2(APP_STATE.currentSection, subsection);
    }
  });
  window.loadSection = loadSection2;
  ...
  window.updateDryRunButton = updateDryRunButton2;
